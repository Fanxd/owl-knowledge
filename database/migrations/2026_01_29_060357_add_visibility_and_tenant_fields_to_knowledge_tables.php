...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        Schema::table('knowledge_categories', function (Blueprint $table) {
            if (!Schema::hasColumn('knowledge_categories', 'visibility')) {
            ...
            if (!Schema::hasColumn('knowledge_categories', 'company_id')) {
            ...
            if (!Schema::hasColumn('knowledge_categories', 'author_id')) {
        ...
        Schema::table('knowledge', function (Blueprint $table) {
            if (!Schema::hasColumn('knowledge', 'visibility')) {
                $table->string('visibility', 32)
                    ->default('public')
                    ->comment('可见性: public=公开, internal=内部可见, private=私有')
                    ->after('metadata');
            ...
            if (!Schema::hasColumn('knowledge', 'author_id')) {
                $table->unsignedBigInteger('author_id')
                    ->nullable()
                    ->comment('创建者用户 ID')
                    ->after('company_id');
